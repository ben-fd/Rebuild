...
export function Header({header, isLoggedIn, cart, deliveryInfo}) {
  ...
  return (
    <header>
      ...
      <div className="z-40" id="notificationContainer"></div>
    </header>
  );
}
          ...
          >
            {viewport == 'desktop' ? (
              <>
                ...
                <ul
                  tabIndex={0}
                  className="menu dropdown-content rounded-box p-2 bg-base-100  shadow-xl"
                >
                  {item.items.map((subitem) => {
                    const subItemUrl = stripURLDomain(
                      subitem,
                      publicStoreDomain,
                      primaryDomainUrl,
                    );
                    return (
                      <li key={viewport + subitem.id + 'child'}>
                        <NavLink
                          end
                          onClick={closeAside}
                          style={activeLinkStyle}
